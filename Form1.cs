...
namespace Pop_o_matic_Trouble_Board_Game
{
    public partial class Form1 : Form
    {
<<<<<<< Updated upstream
=======
        private AI_Player Ai;  // AI player instance
...
// EXIT, ROLL, SAVE, LOAD
        ...
        private bool CanMovePiece(int color, int pieceIndex, int roll)
        {
            ...
            int occupantColor = GetPieceColorAtBoardIndex(targetBoardIndex);
        ...
        private void SendEnemyPieceHome(int enemyColor, int boardIndex)
        {
            ...
            for (int i = 0; i < enemyPieces.Length; i++)
            {
                ...
                // Finding the location of each enemy piece 
                if (enemyPieces[i] == -1)
                {
                    enemyBoardIndex = GetHomeIndexForPiece(enemyColor, i);
                }
                else
                {
                    enemyBoardIndex = GetBoardIndexFromProgress(enemyColor, enemyPieces[i]);
                }

                // Checking if the current enemy piece is on the landed square
                if (enemyBoardIndex == boardIndex)
                {
                    enemyPieces[i] = -1; // Sending that piece to the starting area
                    return;
                }
            }
        }
        ...
        {
            // Checcking through all 4 colors 
            for (int color = 1; color <= 4; color++)
            {
                // Get that color's pieces
                int[] pieces = GetPiecesArray(color);


                for (int i = 0; i < pieces.Length; i++) // Checking all 4 pieces of that color
                {
                    int pieceBoardIndex;


                    // It gets where the piece is lovated 
                    if (pieces[i] == -1)
                    {
                        pieceBoardIndex = GetHomeIndexForPiece(color, i);
                    }
                    else
                        pieceBoardIndex = GetBoardIndexFromProgress(color, pieces[i]);

                    if (pieceBoardIndex == boardIndex) // If a piece is on the path board sqaure, return the color
                        return color;
                }
            }
